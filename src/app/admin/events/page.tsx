...
export default function AdminEventsPage() {
    ...
    function handleAddNew() {
        setCurrentEvent({ title: "", date: "", location: "", desc: "", status: "Open" });
        setIsEditing(true);
    }
    ...
    return (
        <div>
            ...
            <div className="space-y-4">
                {events.map((event) => (
                    <div key={event.id} className="p-6 rounded-xl bg-white/5 border border-white/10 flex justify-between items-center">
                        <div>
                            <h3 className="text-xl font-bold mb-1">{event.title}</h3>
                            <p className="text-sm text-gray-400">{event.date} • {event.location} • {event.status}</p>
                        </div>
                ...
                <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="w-full max-w-lg bg-[#0a0a0a] border border-white/10 rounded-2xl p-8">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-2xl font-bold">{currentEvent.id ? "Edit Event" : "Add Event"}</h2>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                />
                                <label htmlFor="isFeatured" className="text-sm font-medium text-gray-400">Mark as Featured Event</label>
                            </div>
