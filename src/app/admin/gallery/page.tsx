...
import { getGallery, saveGalleryImage, deleteGalleryImage, getCategories, saveCategory, deleteCategory } from "@/actions";
...
export default function AdminGalleryPage() {
    ...
    const [newImage, setNewImage] = useState<any>({ category: "", src: "", size: "small" });
    ...
    async function loadData() {
        const [galleryData, catsData] = await Promise.all([getGallery(), getCategories()]);
        setImages(galleryData);
        setCategories(catsData);
        // Set default category if available
        if (catsData.length > 0) {
            setNewImage((prev: any) => ({ ...prev, category: catsData[0] }));
        }
    }
    ...
    return (
        <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {images.map((img) => (
                    <div key={img.id} className="group relative aspect-square rounded-xl overflow-hidden border border-white/10 bg-gray-800">
                        ...
                        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-between p-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-gray-400 capitalize">{img.size} size</p>
                            </div>
                        </div>
                    </div>
                ))}
                ...
                <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="w-full max-w-md bg-[#0a0a0a] border border-white/10 rounded-2xl p-8">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <button onClick={() => setIsManagingCats(false)} className="text-gray-400 hover:text-white">
                                <X size={24} />
                            </button>
                        </div>
